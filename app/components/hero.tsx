...
export default function Hero({ products, formatPrice }: HeroProps) {
  ...
  return (
    <section id="hero" className="relative overflow-hidden bg-[url('/hero-bg.png')] bg-cover bg-center">
      {/* Egyedi animációk definiálása inline stílusként, hogy biztosan működjön */}
      <style jsx>{`
      ...
      <div className="relative mx-auto flex flex-col md:grid max-w-7xl gap-10 px-6 py-8 md:grid-cols-[1.05fr_0.95fr] md:items-center md:py-16">
        ...
        <div className="max-w-xl mb-10 md:mb-0">
          ...
          <div className="animate-hero-text delay-3 mt-8 flex flex-wrap gap-4">
            <button className="rounded-2xl bg-gradient-to-r from-[#e3936e] to-[#d77d5d] px-7 py-3.5 text-base font-semibold text-white shadow-[0_12px_28px_rgba(215,125,93,0.30)] transition hover:-translate-y-0.5">
              Feltöltöm a képem
            </button>
                  ...
                  <SwiperSlide key={product.id}>
                    <div className="flex flex-col">
                      <div className="relative flex items-center justify-center px-4 md:px-10 min-h-[300px] md:min-h-[400px]">
                        ...
                        <img src={product.image} alt={product.name} className="w-full max-h-[350px] md:max-h-[420px] object-contain drop-shadow-[0_24px_50px_rgba(0,0,0,0.18)]" />
                      </div>
                      <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between px-2">
                        <div className="max-w-md border-l-2 border-[#d17d58] pl-4">
                          ...
                          <h2 className="mt-2 text-2xl font-semibold text-[#2a211d] md:text-[26px]">{product.name}</h2>
                          <div className="mt-3 h-[2px] w-10 bg-[#d17d58]" />
                        </div>
                        <div className="flex shrink-0"><button className="rounded-2xl bg-gradient-to-r from-[#e3936e] to-[#d77d5d] px-6 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5">Részletek →</button></div>
                      </div>
                    </div>
                  </SwiperSlide>
                ))}
