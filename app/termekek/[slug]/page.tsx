...
export default function TermekAdatlap({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f5] text-[#1f1f1f]">
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="lg:flex lg:items-start lg:gap-12">
          ...
          <div className="mt-8 lg:mt-0 lg:w-1/2 max-w-md mx-auto lg:mx-0">
            ...
            {variants.length > 0 && (
              <div className="mt-8">
                <p className="text-[10px] font-black uppercase mb-3 text-[#9a8f84]">Választható méretek</p>
                <div className="grid grid-cols-2 gap-2">
                  {variants.map((v) => (
                    <button
                      key={v.id}
                      onClick={() => setSelectedVariant(v)}
                      className={`p-3 text-xs font-bold rounded-xl border-2 transition-all ${selectedVariant?.id === v.id ? 'border-black bg-white shadow-md' : 'border-gray-100 text-gray-400'}`}
                    >
                      {v.size_name}
                    </button>
                  ))}
            ...
            <button onClick={handleAddToCart} className={`mt-8 w-full rounded-2xl py-4 text-sm font-bold text-white shadow-lg transition-transform active:scale-95 ${isAdded ? "bg-green-600" : "bg-[#e3936e]"}`}>
              {isAdded ? "✓ KOSÁRBAN" : "KOSÁRBA TESZEM"}
            </button>

            {/* --- PRÉMIUM TULAJDONSÁGOK ÉS GARANCIA --- */}
            <div className="mt-12 space-y-8">
              ...
              <section>
                ...
                <p className="text-sm leading-relaxed text-[#4c4742]">
                  {product.description || "Ez a prémium vászonkép modern eleganciát visz otthonodba. Letisztult design és tartós kivitel."}
                </p>
              </section>

              {/* Vászon technikai infók */}
              <section className="bg-white/50 p-6 rounded-[30px] border border-[#d9d5cf]">
                <h4 className="text-[10px] font-black uppercase mb-4 text-[#1f1f1f] tracking-widest text-center">Technikai adatok</h4>
                <ul className="space-y-3">
                  <li className="flex items-center gap-3 text-xs text-[#4c4742]">
                    <span className="text-lg">🎨</span> <strong>Prémium művészi vászon:</strong> Magas grammsúlyú, természetes textúra.
                  </li>
                  <li className="flex items-center gap-3 text-xs text-[#4c4742]">
                    <span className="text-lg">🌲</span> <strong>Borovi fenyő vakráma:</strong> Kezelt, vetemedésmentes fa szerkezet.
                  </li>
                  <li className="flex items-center gap-3 text-xs text-[#4c4742]">
                    <span className="text-lg">📐</span> <strong>Kézi kidolgozás:</strong> Precízen hajtott sarkok és feszes felület.
                  </li>
                  <li className="flex items-center gap-3 text-xs text-[#4c4742]">
                    <span className="text-lg">☀️</span> <strong>UV-álló nyomat:</strong> Évtizedekig tartó, élénk színek.
                  </li>
                </ul>
              </section>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col items-center p-4 bg-white rounded-2xl shadow-sm text-center">
                  ...
                  <p className="text-[9px] font-black uppercase text-[#1f1f1f]">Gyors szállítás</p>
                  <p className="text-[8px] text-gray-400 mt-1">2-4 munkanap</p>
                </div>
                <div className="flex flex-col items-center p-4 bg-white rounded-2xl shadow-sm text-center">
                  <span className="text-xl mb-2">🛡️</span>
                  <p className="text-[9px] font-black uppercase text-[#1f1f1f]">100% Garancia</p>
                  <p className="text-[8px] text-gray-400 mt-1">Pénzvisszafizetés</p>
                </div>
                <div className="flex flex-col items-center p-4 bg-white rounded-2xl shadow-sm text-center">
                  <span className="text-xl mb-2">↩️</span>
                  <p className="text-[9px] font-black uppercase text-[#1f1f1f]">30 napos csere</p>
                  <p className="text-[8px] text-gray-400 mt-1">Kérdés nélkül</p>
                </div>
                <div className="flex flex-col items-center p-4 bg-white rounded-2xl shadow-sm text-center">
                  <span className="text-xl mb-2">🔒</span>
                  <p className="text-[9px] font-black uppercase text-[#1f1f1f]">Biztonságos</p>
                  <p className="text-[8px] text-gray-400 mt-1">SSL titkosítás</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* 3D MODAL RÉSZ (Változatlanul hagytam) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 animate-in fade-in duration-300">
          ...
          <div className="relative w-full h-full md:h-[85vh] max-w-5xl bg-[#f8f8f6] md:rounded-[32px] overflow-hidden shadow-2xl flex flex-col">
            <div className="relative p-5 border-b border-gray-200 flex justify-between items-center bg-white z-[110]">
              ...
              <button onClick={() => setIsModalOpen(false)} className="bg-black text-white w-10 h-10 rounded-xl flex items-center justify-center shadow-md active:scale-90 transition-all font-bold">✕</button>
            </div>
            <div className="flex-1 w-full relative">
              <CanvasViewer modelUrl={product.model_url || "/models/canvas-portrait.glb"} />
            </div>
          </div>
        </div>
      )}
