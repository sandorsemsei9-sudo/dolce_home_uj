...
// Dynamic import any típussal a rugalmasságért
...
export default function TermekAdatlap({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const masterGlbPath = "/models/canvas-landscape.glb"; 
            ...
            {variants.length > 0 && (
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {variants.map((v) => (
                    <button key={v.id} onClick={() => setSelectedVariant(v)} className={`p-3 text-xs font-bold rounded-xl border-2 transition-all ${selectedVariant?.id === v.id ? 'border-black bg-white shadow-md' : 'border-gray-100 text-gray-400'}`}>
                      {v.size_name}
                    </button>
                  ))}
                </div>
          ...
          <div className="relative w-full h-full md:h-[85vh] max-w-5xl bg-[#f8f8f6] md:rounded-[32px] overflow-hidden shadow-2xl flex flex-col">
            <div className="p-5 border-b flex justify-between items-center bg-white z-[110]">
              <div>
                ...
                <p className="text-[10px] text-gray-400 font-medium uppercase">3D Előnézet</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="bg-black text-white w-10 h-10 rounded-xl font-bold">✕</button>
            </div>
            <div className="flex-1 w-full relative">
              <CanvasViewer 
                modelUrl={masterGlbPath} 
                iosModelUrl={isIOS ? iosUsdzPath : ""} 
                textureUrl={product.cover_image}
              />
            </div>
          </div>
        </div>
